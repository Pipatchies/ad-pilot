...
export default function InvoicesTable({
  ...
  const [selectedVendorForModal, setSelectedVendorForModal] = useState<InvoiceWithVendor | null>(
    null,
  );
  ...
  const columns: ColumnDef<InvoiceWithVendor>[] = [
...

    {
      ...
      cell: ({ row }) => `${(row.getValue('htprice') as number).toLocaleString('fr-FR')} €`,
    },

    {
      accessorKey: 'ttcprice',
      header: sortableHeader('Montant TTC'),
      cell: ({ row }) => `${(row.getValue('ttcprice') as number).toLocaleString('fr-FR')} €`,
    },
  ...
  return (
    <>
      <DataTable
        ...
        defaultSort={dateSort ? { id: 'startDate', desc: dateSort === 'desc' } : undefined}
      />
      {viewingInvoice && viewingInvoice.url && (
        <MediaViewerModal
          ...
          hasNext={invoices.findIndex((i) => i._id === viewingInvoice._id) < invoices.length - 1}
