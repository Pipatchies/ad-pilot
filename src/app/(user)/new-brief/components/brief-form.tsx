    ...
    <section>
      ...
      <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col relative">
                      <FormLabel className='text-lg font-semibold'>Période</FormLabel>
                        <div
                          onMouseEnter={handleMouseEnter}
                          onMouseLeave={handleMouseLeave}
                          className={cn(
                            "w-full border border-[#A5A4BF] rounded-sm py-2 px-5 flex items-center justify-between",
                        ...
                        >
                          <span
                            className={cn(
                              "text-base italic",
                              field.value?.from && field.value?.to ? "text-primary" : "text-primary/50"
                            )}
                          >
                            {field.value?.from && field.value?.to
                              ? `${format(field.value.from, "dd MMM yyyy", { locale: fr })} - ${format(field.value.to, "dd MMM yyyy", { locale: fr })}`
                    ...
                    {calendarOpen && (
                      <div
                        className="absolute top-full z-50 mt-2 bg-white rounded-sm shadow border"
                        onMouseEnter={handleMouseEnter}
                        onMouseLeave={handleMouseLeave}
                      >
                        <Calendar
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className='text-lg font-semibold'>Cible</FormLabel>
                      <FormControl>
                        <Input placeholder="Renseignez votre cible" className="!text-base md:text-base italic placeholder:text-primary/50 rounded-sm border-[#A5A4BF] p-5" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className='text-lg font-semibold'>Territoire</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <SelectTrigger className="w-full text-base italic rounded-sm border border-[#A5A4BF] p-5">
                            <SelectValue placeholder={
                                <span className="text-primary/50 italic">Sélectionnez un type de territoire</span>
                              } />
                          </SelectTrigger>
                        </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className='text-lg font-semibold'>Villes</FormLabel>
                      <FormControl>
                        <Input placeholder="Précisez les villes" className="!text-base md:text-base italic placeholder:text-primary/50 rounded-sm border-[#A5A4BF] p-5" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Précisez votre budget" className="!text-base md:text-base italic placeholder:text-primary/50 rounded-sm border-[#A5A4BF] p-5" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      <FormLabel className='text-lg font-semibold'>Objectifs</FormLabel>
                      <Popover open={objectifsOpen} onOpenChange={setObjectifsOpen}>
                            ...
                            >
                              {field.value.length > 0 ? (
                                field.value.length > 2 ? (
                                  <span>{field.value.length} objectifs sélectionnés</span>
                                ) : (
                                  <div className="flex gap-1 flex-wrap">
                                    {field.value.map((objectif) => (
                                      <Badge key={objectif} className="bg-primary text-white rounded-sm px-2 py-1 text-sm">
                                        {objectifs.find((o) => o.value === objectif)?.label}
                                      </Badge>
                                    ))}
                                  </div>
                                )
                            ...
                            <CommandList>
                              <CommandGroup>
                                {objectifs.map((objectif) => {
                                  const isSelected = field.value.includes(objectif.value)
                                  return (
                                    <CommandItem
                                      key={objectif.value}
                                      onSelect={() => {
                                        const updatedValue = isSelected
                                          ? field.value.filter((val) => val !== objectif.value)
                                    ...
                                    >
                                      <div className="flex items-center gap-2">
                                        <Checkbox checked={isSelected} onCheckedChange={() => {}} className="mr-2" />
                                        {objectif.label}
                                      </div>
                                    </CommandItem>
                                  )
            ...
            <div className="space-y-4">
              <FormLabel className="text-lg font-semibold">Média(s)</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <Popover open={mediaTypeOpen} onOpenChange={setMediaTypeOpen}>
                      <PopoverTrigger asChild>
                        <FormControl>
                          <Button
                            variant="ghost"
                          ...
                          >
                            {field.value.length > 0 ? (
                              field.value.length > 2 ? (
                                <span>{field.value.length} types de média sélectionnés</span>
                              ) : (
                                <div className="flex gap-1 flex-wrap">
                                  {field.value.map((mediaType) => (
                                    <Badge key={mediaType} className="bg-primary text-white rounded-sm px-2 py-1 text-sm">
                      ...
                      <PopoverContent className="w-[--radix-popover-trigger-width] p-0 mt-2 border border-[#A5A4BF] shadow-md rounded-sm" align="start">
                        <Command>
                              ...
                              {mediaTypes.map((mediaType) => {
                                ...
                                return (
                                  <CommandItem
                                    ...
                                    onSelect={() => {
                                      const updatedValue = isSelected
                                        ? field.value.filter((val) => val !== mediaType.value)
              ...
              {selectedMediaTypes.includes("tv") && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <Popover open={diffusionTVOpen} onOpenChange={setDiffusionTVOpen}>
                        <PopoverTrigger asChild>
                          <FormControl>
                            <Button
                              variant="ghost"
                            ...
                            >
                              {(field.value || []).length  > 0 ? (
                                (field.value || []).length  > 2 ? (
                                  <span>{(field.value || []).length } diffusions TV sélectionnées</span>
                                ) : (
                                  <div className="flex gap-1 flex-wrap">
                                    {field.value?.map((val) => (
                                      <Badge key={val} className="bg-primary text-white rounded-sm px-2 py-1 text-sm">
                                        {diffusionTVs.find((o) => o.value === val)?.label}
                                      </Badge>
                                    ))}
                                  </div>
                                )
                    ...
                    <FormItem>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          ...
                          <SelectValue placeholder={
                                <span className="text-primary/50 italic">Sélectionnez le type d'affichage</span>
                              } />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent className="w-full text-base italic rounded-sm border border-[#A5A4BF] text-primary text-base">
              ...
              {selectedMediaTypes.includes("radio") && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <Popover open={diffusionRadioOpen} onOpenChange={setDiffusionRadioOpen}>
                        <PopoverTrigger asChild>
                          <FormControl>
                            <Button
                              variant="ghost"
                            ...
                            >
                              {(field.value || []).length  > 0 ? (
                                (field.value || []).length  > 2 ? (
                                  <span>{(field.value || []).length } diffusions radio sélectionnées</span>
                                ) : (
                                  <div className="flex gap-1 flex-wrap">
                                    {field.value?.map((val) => (
                                      <Badge key={val} className="bg-primary text-white rounded-sm px-2 py-1 text-sm">
                                        {diffusionRadios.find((o) => o.value === val)?.label}
                                      </Badge>
                                    ))}
                                  </div>
                                )
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className='text-lg font-semibold'>Votre brief</FormLabel>
                    <FormControl>
                      <Textarea
