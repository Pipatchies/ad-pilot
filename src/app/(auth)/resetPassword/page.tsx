      ...
      <div className='w-full max-w-xl space-y-8'>
        ...
        <div className='rounded-2xl border border-gray-200/70 shadow-sm p-6 md:p-8'>
          <div className='text-center mb-6'>
            <h1 className='text-2xl font-bold tracking-tight'>Réinitialiser votre mot de passe</h1>
            ...
            <div className='mt-4 flex items-center justify-center gap-2'>
              <span
                aria-hidden
                className={`h-2 w-2 rounded-full ${step === 'forgot' ? 'bg-black' : 'bg-gray-300'}`}
              />
              <span
                aria-hidden
                className={`h-2 w-2 rounded-full ${step === 'verify' ? 'bg-black' : 'bg-gray-300'}`}
              />
            </div>
          </div>
          ...
          {step === 'forgot' ? (
            <Form {...forgotForm}>
              <form onSubmit={forgotForm.handleSubmit(sendCode)} className='space-y-4'>
                <FormField
              ...
              <form onSubmit={verifyForm.handleSubmit(verify)} className='space-y-4'>
                <div className='grid gap-4'>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className='sr-only'>Nouveau mot de passe</FormLabel>
                        <FormControl>
                          <Input
                            type='password'
                ...
                <div className='flex gap-3'>
                  <Button
                    type='submit'
                    disabled={
                      pending || !emailValue?.trim() || !codeValue?.trim() || !newPwdValue?.trim()
                    }
                ...
                <p className='text-center text-xs text-gray-500'>
                  ...
                  <span className='font-medium'>recharge la page pour renvoyer</span>.
                </p>
              </form>
            </Form>
          )}
