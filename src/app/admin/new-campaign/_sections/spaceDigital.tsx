...
export default function SpaceDigital({ campaignId }: { campaignId?: string }) {
  ...
  const handleUpload = async (file: File, onChange: (value: any) => void) => {
    try {
      ...
      const folder = campaignId ? `campaigns/${campaignId}/digital` : `campaigns/temp`;
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='w-full'>
                <FormLabel className='text-lg font-semibold mb-2'>Document de récap</FormLabel>
                <FormControl>
                  <div className='relative'>
                    <Input
                      readOnly
                      value={
                        field.value?.name || (typeof field.value === 'string' ? field.value : '')
                      }
                      placeholder={uploading ? 'Upload en cours...' : 'analyse.pdf'}
                      ...
                      onClick={() =>
                        !uploading && document.getElementById('hiddenDigitalInput')?.click()
                      }
                    />
                    <SvgUploder
